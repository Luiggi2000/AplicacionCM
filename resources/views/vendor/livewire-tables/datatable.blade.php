    ...
    <x-livewire-tables::wrapper :component="$this" :tableName="$tableName">
        ...
        <x-livewire-tables::table>
            <x-slot name="thead">
                ...
                @foreach ($this->columns() as $index => $column)
                    ...
                    <x-livewire-tables::table.th
                        wire:key="{{ $tableName . '-table-head-' . $index }}"
                        :column="$column"
                        :index="$index"
                    />
                @endforeach
            ...
            @if ($this->hasDisplayLoadingPlaceholder())
                <x-livewire-tables::includes.loading colCount="{{ count($this->columns()) + 1 }}" />
            @endif
            ...
            @forelse ($rows as $rowIndex => $row)
                <x-livewire-tables::table.tr
                    wire:key="{{ $tableName }}-row-wrap-{{ $row->{$this->getPrimaryKey()} }}"
                    :row="$row"
                    :rowIndex="$rowIndex"
                >
                    <x-livewire-tables::table.td.reorder
                        ...
                        :rowID="$tableName . '-' . $row->{$this->getPrimaryKey()}"
                        :rowIndex="$rowIndex"
                    />
